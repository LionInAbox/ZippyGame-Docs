...
[data-md-color-scheme="slate"] .color-row.clicked {
...
#copyTooltip {
  position: fixed;
  background: rgba(38, 48, 73, 0.77);
  color: #fff;
  text-shadow: 
  0px 0px 5px rgba(38, 48, 73,1),
  0px 0px 4px rgba(38, 48, 73,1),
  0px 0px 3px rgba(38, 48, 73,1);
  font-size: 0.65rem;
  padding: 1px 4px;
  border-radius: 8px;
  pointer-events: none;
...
function copyColor(row, tuple) {
  ...
  setTimeout(function() { row.classList.remove('clicked'); }, 600);
  ...
  _tooltipTimer = setTimeout(function() { tip.classList.remove('show'); }, 800);
}
// tool tip disappears when you move the cursor:
document.addEventListener('mousemove', function(e) {
  var tip = document.getElementById('copyTooltip');
  if (!tip.classList.contains('show')) return;
  var dx = e.clientX - _clickX;
  var dy = e.clientY - _clickY;
